...
namespace contract
{
    public partial class CustomSpGrid : DataGridView
    {
        public CustomSpGrid()
        {
            InitializeComponent();
                ...
                for (int i = 0; i < this.Columns.Count; i++)
                {
                    if (this.GetCellDisplayRectangle(i, -1, false).X != 0)
                    {
                        ...
                        if (i == 7 || i == 8 || i == 9 || i == 10 || i == 11)
                        {
                            if (i == 7)
                            {
                                int _width = this.GetCellDisplayRectangle(i, -1, false).Width + this.GetCellDisplayRectangle(i + 1, -1, false).Width + this.GetCellDisplayRectangle(i + 2, -1, false).Width + this.GetCellDisplayRectangle(i + 3, -1, false).Width + this.GetCellDisplayRectangle(i + 4, -1, false).Width;
                                e.Graphics.DrawRectangle(pLine, r.X - 1, r.Y - 1, _width, r.Height / 2);
