...
namespace contract
{
    public partial class A_RPT_HZ1 : Form
    {
        private string name;
        ...
        public A_RPT_HZ1(string name)
        {
            InitializeComponent();
            this.name = name;
        }

        private void A_RPT_HZ_Load(object sender, EventArgs e)
        {
            if (ClassConstant.USER_NAME == "于萍")
            {
                this.toolStripComboBox3.Enabled = true;
                this.toolStripComboBox3.Items.AddRange(new string[]{
                            "01:沈阳铸锻工业有限公司",
                            "0101:沈阳铸锻工业有限公司本部公司",
                            "0102:沈阳铸锻工业有限公司铸钢公司",
                            "0103:沈阳铸锻工业有限公司锻造公司",
                            "0104:沈阳铸锻工业有限公司热处理公司",
                            "0105:沈阳铸锻工业有限公司特钢公司",
                            "0106:沈阳铸锻工业有限公司铸铁公司",
                            "0107:沈阳铸锻工业有限公司机加公司",
                            "0108:沈阳铸锻工业有限公司动能公司",
            ...
            {
                this.toolStripComboBox3.Text = ClassConstant.DW_ID + ":" + ClassConstant.DW_NAME;
            ...
            foreach (DataRow r in DBAdo.DtFillSql("SELECT LID+':'+LNAME FROM ALX WHERE LEN(LID)=2 AND LID NOT LIKE '03%'").Rows)
            {
                this.toolStripComboBox4.Items.Add(r[0].ToString());
            }
        ...
        private void EXCEL_FKMX()
        {
            ...
            excel.get_Range("B4", "c6").Value = "合同种类";
            excel.get_Range("D4", "G4").Value = "合同总额";
            excel.get_Range("H4", "K4").Value = (this.toolStripComboBox4.Text.Substring(0, 2) == "02" ? "已收货款" : "已付货款");
            excel.get_Range("L4", "M4").Value = (this.toolStripComboBox4.Text.Substring(0, 2) == "02" ? "未收货款" : "未付货款");
            excel.get_Range("N4", "Q4").Value = (this.toolStripComboBox4.Text.Substring(0, 2) == "02" ? "已开发票金额" : "已收发票金额");
            excel.get_Range("R4", "S4").Value = (this.toolStripComboBox4.Text.Substring(0, 2) == "02" ? "未开票金额" : "未收票金额");
            ...
            excel.get_Range("a3", "a3").Value = this.toolStripComboBox3.Text + "      " + this.toolStripComboBox1.Text + "年" + this.toolStripComboBox2.Text + "月";
            ...
            DataTable dt = DBAdo.DtFillSql("SELECT LID,LNAME FROM ALX WHERE  " + (str == "01" ? "  LID LIKE '03__' OR " : "") + " LID LIKE '" + ClassCustom.codeSub(this.toolStripComboBox4.Text) + "__'");
            int index = 0;
        ...
        private void toolStripButton1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    string filter = (this.toolStripComboBox3.Text == "01:沈阳铸锻工业有限公司" ? "" : " AND HDW = '" + ClassCustom.codeSub(this.toolStripComboBox3.Text) + "'") + " AND HLX = '" + ClassCustom.codeSub(r[1].ToString()) + "' AND " + (r[2].ToString() == "内部" ? "HKH LIKE '01%'" : "HKH NOT LIKE '01%'");
                    ...
                    string lastYear1 = DBAdo.ExecuteScalarSql(sql_fir + " WHERE 1=1 AND year(date) < " + this.toolStripComboBox1.Text + " " + filter + " AND TYPE = '" + (ClassCustom.codeSub(this.toolStripComboBox4.Text) == "02" ? "回款" : "付款") + "'").ToString();
                    string thisMonth1 = DBAdo.ExecuteScalarSql(sql_fir + " WHERE 1=1 AND year(date) = " + this.toolStripComboBox1.Text + "  AND MONTH(date) = " + this.toolStripComboBox2.Text + filter + " AND TYPE = '" + (ClassCustom.codeSub(this.toolStripComboBox4.Text) == "02" ? "回款" : "付款") + "'").ToString();
                    string thisYear1 = DBAdo.ExecuteScalarSql(sql_fir + " WHERE 1=1 AND  year(date) = " + this.toolStripComboBox1.Text + " AND MONTH(date) <= " + this.toolStripComboBox2.Text + " " + filter + " AND TYPE = '" + (ClassCustom.codeSub(this.toolStripComboBox4.Text) == "02" ? "回款" : "付款") + "'").ToString();
                    string all1 = DBAdo.ExecuteScalarSql(sql_fir + " WHERE 1=1 AND (year(date) < " + this.toolStripComboBox1.Text + "OR ( year(date) = " + this.toolStripComboBox1.Text + " AND MONTH(date) <= " + this.toolStripComboBox2.Text + ") )" + filter + " AND TYPE = '" + (ClassCustom.codeSub(this.toolStripComboBox4.Text) == "02" ? "回款" : "付款") + "'").ToString();


                    string lastYear2 = DBAdo.ExecuteScalarSql(sql_fir + " WHERE 1=1 AND year(date) < " + this.toolStripComboBox1.Text + " " + filter + " AND TYPE = '" + (ClassCustom.codeSub(this.toolStripComboBox4.Text) == "02" ? "销项发票" : "进项发票") + "'").ToString();
                    string thisMonth2 = DBAdo.ExecuteScalarSql(sql_fir + " WHERE 1=1 AND year(date) = " + this.toolStripComboBox1.Text + "  AND MONTH(date) = " + this.toolStripComboBox2.Text + filter + " AND TYPE = '" + (ClassCustom.codeSub(this.toolStripComboBox4.Text) == "02" ? "销项发票" : "进项发票") + "'").ToString();
                    string thisYear2 = DBAdo.ExecuteScalarSql(sql_fir + " WHERE 1=1 AND  year(date) = " + this.toolStripComboBox1.Text + " AND MONTH(date) <= " + this.toolStripComboBox2.Text + " " + filter + " AND TYPE = '" + (ClassCustom.codeSub(this.toolStripComboBox4.Text) == "02" ? "销项发票" : "进项发票") + "'").ToString();
                    string all2 = DBAdo.ExecuteScalarSql(sql_fir + " WHERE 1=1 AND (year(date) < " + this.toolStripComboBox1.Text + "OR ( year(date) = " + this.toolStripComboBox1.Text + " AND MONTH(date) <= " + this.toolStripComboBox2.Text + ") )" + filter + " AND TYPE = '" + (ClassCustom.codeSub(this.toolStripComboBox4.Text) == "02" ? "销项发票" : "进项发票") + "'").ToString();
